...
**HTML tables** are **used to structure and display tabular data such as schedules, price lists, or statistics in a rigid grid of rows and columns.** An HTML table is created using the `<table>` element, with horizontal rows defined by `<tr>` tags, and individual cells defined by `<th>` (header cells) or `<td>` (data cells) tags
...
<img src="Assets/table.png" alt="row-span and colspan image.png" >